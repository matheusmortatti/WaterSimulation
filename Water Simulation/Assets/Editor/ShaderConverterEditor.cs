...
public class ShaderConverterEditor : EditorWindow
{
    ...
    static void Initialize()
    {
        if (GameObject.Find("CodeGenerator") == null)
        {
            ...
            obj.AddComponent<ShaderToyConverter>();
        }
    ...
    void CreateShader()
    {
        ...
        if (ShaderToyConverter.instance != null || Replace)
        {
            var sr = File.CreateText(path + fileName);

            sr.WriteLine("");
            ShaderToyConverter.instance.ShaderName = shaderName;
            sr.WriteLine(ShaderToyConverter.instance.Convert(text));
            sr.Close();
        }
